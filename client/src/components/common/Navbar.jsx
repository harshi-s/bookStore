...
export const Navbar = () => {
  ...
  const navItems = [
    ...
    { link: 'Shop', path: '/shop' },
    { link: 'Cart', path: '/cart' }, // Added Cart Link
    { link: 'Upload Your Book', path: '/admin/dashboard/upload' },
    { link: 'Blog', path: '/blog' },
  ];
  ...
  return (
    <header className="w-full bg-transparent fixed top-0 left-0 right-0 transition-all ease-in duration-300">
      <nav
        className={`py-4 lg:px-24 px-4 ${isSticky ? 'sticky top-0 left-0 right-0 bg-blue-400' : ''}`}
      >
        <div className="flex justify-between items-center text-base gap-8">
          ...
          <ul className="md:flex space-x-12 hidden">
            {navItems.map(({ link, path }) => (
              <Link key={path} to={path} className="text-black uppercase cursor-pointer hover:text-blue-700">
                {link}
              </Link>
            ))}
            {user && (
            ...
            {user ? (
              <div>
                <img
                  src={user?.photoURL}
                  alt=""
                  className="w-[48px] p-1 rounded-full border border-gray-700"
                />
              </div>
            ) : (
        ...
        <div className={`space-y-4 px-4 mt-16 py-7 bg-blue-700 ${isMenuOpen ? 'block fixed top-0 right-0 left-0 ' : 'hidden'}`}>
          {navItems.map(({ link, path }) => (
            <Link key={path} to={path} className="block text-base text-white uppercase cursor-pointer hover:text-black">
              {link}
            </Link>
          ))}
          {user && (
            <div className="flex gap-3 items-center">
              ...
              <p className="flex flex-col justify-start">
                {user?.displayName}
                <Link to={'/log-out'} className="text-sm text-red-700 underline">
                  logout
                </Link>
              </p>
            </div>
